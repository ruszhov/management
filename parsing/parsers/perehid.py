...
def perehid(filename, contractor):
    ...
    operator_id = operator_create('Перехід', contractor)
    header_row = 9
    table_first_row = 10
    light_presence = 'Да'
    sold_presence = 'бронь'
    ...
    if check_status.status == 1:
        try:
            ...
            kwargs = {
                'kod_col': 'Код плоскости',
                'url_col': 'Фото/ схема (ссылка)',
                'city_col': 'Город',
                ...
                'size_col': 'Формат',
                'side_col': 'Сто- ро- на',
                'light_col': 'Осве- щение',
                'ots_col': 'OTS',
                'grp_col': 'GRP',
                'doors_col': 'Код DOORS',
                'price_col': 'Прайс за месяц, без НДС'
            }
            args = ('Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь')
            ...
            for i in range(table_first_row, max_row + 1):
                ...
                if kod is not None and kod != '' and len(kod) <= 10:
                    raw_kod_doors = sheet.cell(row=i, column=col_idx['doors_col'][0]).value
                    kod_doors_int = raw_kod_doors if isinstance(raw_kod_doors, int) else None
                    ...
                    ots = raw_ots if raw_ots != '' and raw_ots is not None and isinstance(raw_ots, float) or isinstance(raw_ots, int) else None

                    raw_grp = sheet.cell(row=i, column=col_idx['grp_col'][0]).value
                    grp = raw_grp if raw_grp != '' and raw_grp is not None and isinstance(raw_grp, float) or isinstance(raw_grp, int) else None
